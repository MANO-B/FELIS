    ...
    model {
      ybef_exp ~ weibull(shape_exp, exp(scale_exp + xfactor * factor + disease_effect + xfactor2 * factor2));
    ...
    generated quantities {
      ...
      for (i in 1:Nobs) {
        ...
        y_bef =  weibull_rng(shape_exp, exp(scale_exp + factor2 + disease_effect[i]));
        if (y_bef <= Median_2) {
      ...
      for (i in 1:Ncen) {
        ...
        y_bef =  weibull_rng(shape_exp, exp(scale_exp + factor2 + disease_effect[Nobs + i]));
        if (y_bef <= Median_2) {
