  ...
  with(analysis_env, {
    withProgress(message = sample(nietzsche)[1], {
      ...
      if(nrow(Data_case_target)>0){
        ...
        if("Panel" %in% colnames(Data_forest_tmp__$Panel)){
        ...
        if("Enroll_date" %in% colnames(Data_forest_tmp__$Enroll_date)){
        ...
        Data_ML_full = Data_ML_full %>%
          dplyr::filter(PS != "Unknown" & Sex != "Unknown" & Smoking_history != "Unknown" & Alcoholic_history != "Unknown" &
                        # !is.na(Enroll_date) &
                        !is.na(time_diagnosis_enroll)
                        ) %>%
          dplyr::mutate(
            PS = case_when(
              PS == 0 ~ "0",
          ...
          if(input$new_analysis =="No, use the previous dataset" & input$intermediate_file != "No" & file.exists(file.path(tempdir(), "glm_param.qs"))){
            ...
            if(FLAG){
              ...
              if(input$machine_learning == "Yes"){
                ...
                factor_cols = factor_cols[factor_cols %in% c("PS", "Panel", "Histology", "Best_effect", "Lines", "Enroll_date")]
...
selected_df <- reactive({
  ...
  } else if (input$nomogram_type_4 == "GMT") {
...
output$table_prediction <- DT::renderDataTable(server = FALSE,{
  req(selected_df())
  create_datatable_with_confirm(selected_df(), "FELIS downloaded raw data in ROC curve of nomogram tab")
})
...
output$figure_DCA_pre_CGP_1 = renderPlot({
  ...
  if (input$nomogram_type_3 == "EP_treat") {
    ...
    xlim_value <- c(0, 1)
    dca_thresholds = seq(0, 1, 0.01)
  } else if (input$nomogram_type_3 == "GMT") {
    req(OUTPUT_DATA$table_prediction_df_cv_pred_GMT)
...
output$histology_nomogram_pre_CGP = renderText({
  ...
  } else if (input$nomogram_type == "GMT") {
  ...
  validate(
    need(!is.null(nomogram_obj$Histology$points), "")
  )
