...
#  - Estimate TOTAL effect on OS by Counterfactual Distributions (do-intervention)
#  - Adjusted CI using Effective Sample Size (ESS) to prevent underestimation
#  - Strict Spline Extrapolation Clamping to prevent Inf / NA errors
# =========================================================================
...
# -------------------------------------------------------------------------
# SAFER simulation-time generator (Inf/NA prevention included)
# -------------------------------------------------------------------------
gen_sim_times_safe <- function(fit, newdata, dist_type = NULL) {
  ...
  # --- Guard linear predictor ---
  xb <- pmax(pmin(xb, 15), -15)
  ...
  if (dist_type %in% c("weibull", "weibullPH", "weibullph")) {
    ...
    if(is.finite(shape) && shape > 0) sim_t <- rweibull(n, shape = shape, scale = exp(mu))
  } else if (dist_type %in% c("llogis", "loglogistic")) {
    base <- if ("scale" %in% rownames(res_m)) log(as.numeric(res_m["scale", "est"])) else 0
    mu <- base + xb
    shape <- as.numeric(res_m["shape", "est"])
    if(is.finite(shape) && shape > 0) sim_t <- flexsurv::rllogis(n, shape = shape, scale = exp(mu))
    ...
    if(is.finite(sigma) && sigma > 0) sim_t <- flexsurv::rgengamma(n, mu = mu, sigma = sigma, Q = Q)
  }

  # --- Final Fallback: Guard against Inf and NAs ---
  sim_t[is.na(sim_t)] <- median(sim_t, na.rm=TRUE)
  if(is.na(sim_t[1])) sim_t <- rep(365.25, n)
  sim_t <- pmin(sim_t, 365.25 * 100) # Cap at 100 years max
...
ratio_ci_from_samples <- function(x0, x1, n_eff) {
  ...
  # Calculate Standard Error of Median (log scale)
  calc_se_log_med <- function(x, m) {
    ...
    if (!is.null(den)) {
      ...
      if (is.finite(f_lm) && f_lm > 0) {
        f_m <- f_lm / m
        # Use n_eff (Effective Sample Size) to accurately reflect true uncertainty
        se_m <- 1 / (2 * sqrt(n_eff) * f_m)
        return(se_m / m)
      }
    }
    # Fallback to robust SD
    s <- mad(lx)
    if (s == 0) s <- sd(lx)
    if (s == 0) s <- 0.1
...
compute_prop_afs_counterfactual <- function(pseudo_base, fit_t1, fit_t2, fit_t1res, ns_obj, ns_colnames, Data_cgp_logT1_resid, n_eff) {
  ...
  sim_os_do <- function(df) {
    ...
    # CLAMP: outer-range limits based on training data to prevent spline extrapolation explosion
...
run_sim_iteration <- function(N_target, True_AF_X, Mut_Freq, True_Med, True_Shape,
  ...
  # -------------------------------------------------------------
  # Counterfactual Total Effect via Median Ratio
  # -------------------------------------------------------------
  cf <- tryCatch(
    compute_prop_afs_counterfactual(
      ...
      Data_cgp_logT1_resid = Data_cgp$logT1_resid,
      n_eff = ESS # Penalize CI using Effective Sample Size
    ),
    error = function(e) NULL
  )
