...
#   + Everything else kept as-is as much as possible
# =========================================================================
...
# =========================================================
# find_censoring_param (UNCHANGED)
# =========================================================
...
# =========================================================
# gen_sim_times(): weibull / llogis / gengamma (SAFER)
#   - try predict(type="lp")
#   - if fails, compute LP manually from fit$res
# =========================================================
...
# =========================================================
# calculate_calibrated_iptw (UNCHANGED; 1:5 points)
# =========================================================
...
# =========================================================
# Bootstrap CI for Proposed (Total Effect)
#   - Only for CI replacement, point estimate stays from original
#   - Percentile CI on AF scale
#   - Robust to factor-level dropping
# =========================================================
bootstrap_prop_ci <- function(Data_cgp, ref_surv_list, valid_covs,
                              B = 200, n_pseudo = 10000, ns_df = 3) {
  ...
  for (b in 1:B) {
    ...
    # re-calc IPTW (same function, same 1:5)
    ...
    # final model (keep llogis as in your pipeline)
  ...
  ciX <- q025_975(store$AF_X)
  ciA <- q025_975(store$AF_Age)
  ciS <- q025_975(store$AF_Sex)
  ciR <- q025_975(store$AF_READ)
  ciC <- q025_975(store$AF_COADREAD)

  list(
    AF_X_L = ciX[1], AF_X_U = ciX[2],
    AF_Age_L = ciA[1], AF_Age_U = ciA[2],
    AF_Sex_L = ciS[1], AF_Sex_U = ciS[2],
    AF_READ_L = ciR[1], AF_READ_U = ciR[2],
    AF_COADREAD_L = ciC[1], AF_COADREAD_U = ciC[2],
...
#   - 1:5 calibration 유지
#   - T2 model: gengamma
#   - Proposed is TOTAL EFFECT (T1+T2) via G-computation
#   - Proposed CI replaced by Bootstrap percentile CI (B=200)
# =========================================================================
run_sim_iteration <- function(N_target, True_AF_X, Mut_Freq, True_Med, True_Shape,
  ...
  # --- Step 3: Calibrated G-computation (TOTAL EFFECT) ---
  ...
  # --- Bootstrap CI (B=200) ONLY for Proposed ---
  boot_ci <- bootstrap_prop_ci(
    Data_cgp = Data_cgp,
...
# -------------------------------------------------------------------------
# UI Observers and Rendering Logic (RESTORED FULLY)
# -------------------------------------------------------------------------
...
observeEvent(input$run_sim, {
  ...
  output$sim_survival_plot <- renderPlot({
    ...
    ggplot(plot_df, aes(x = Time, y = Survival, color = Model, linetype = Model)) +
      ...
      labs(title = "Tamura & Ikegami Model Ver 2.3.2 Validation",
...
observeEvent(input$run_sim_multi, {
  ...
  # AF aggregation: Mean, MSE, Coverage Rate
  ...
  # =========================================================================
  # Figures (AS-IS from your script)
  # =========================================================================

  # --- Fig 1: mean survival curves ---
