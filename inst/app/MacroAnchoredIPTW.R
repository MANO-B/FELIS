# =========================================================================
# Server Logic for Simulation Study (Tamura & Ikegami Model Ver 2.3.2 + Bootstrap CI)
#   - Keep 1:5 calibration
#   - T2 model: gengamma
#   - Total Effect (X affects both T1 and T2 -> OS total effect via G-computation)
#   - Proposed Method CI: Bootstrap percentile CI (B=200)
# =========================================================================
...
# =========================================================
# Censoring parameter finder (unchanged)
# =========================================================
...
# =========================================================
# gen_sim_times(): weibull / llogis / gengamma
#   - safer: try predict(type="lp"), if fails compute lp manually
# =========================================================
gen_sim_times <- function(fit, newdata, dist_type = NULL) {
  ...
  # 1) Try predict(type="lp")
  mu <- tryCatch(
    predict(fit, newdata = newdata, type = "lp"),
    error = function(e) NULL
  )

  # 2) Fallback: manual linear predictor on location (mu)
  if (is.null(mu)) {
    res_m <- fit$res
    # baseline location
    if (dist_type %in% c("gengamma", "gen_gamma", "generalized gamma")) {
      if (!("mu" %in% rownames(res_m))) stop("gengamma: baseline mu not found in fit$res")
      base_mu <- as.numeric(res_m["mu", "est"])
    } else {
      if (!("scale" %in% rownames(res_m))) stop("dist: baseline scale not found in fit$res")
      base_mu <- log(as.numeric(res_m["scale", "est"]))  # location = log(scale) in AFT for many flexsurv dists
    }

    lp <- rep(0, n)
    # add known covariates if present
...
# =========================================================
# Calibration IPTW (1:5 points) - unchanged
# =========================================================
...
# =========================================================
# Helpers: extract AF for all vars from flexsurvreg fit
# =========================================================
...
# =========================================================
# Proposed (Total Effect) point estimate + bootstrap CI (B=200)
#   - percentile CI on AF scale
# =========================================================
bootstrap_proposed_ci <- function(Data_cgp, ref_surv_list, Age, T_true, valid_covs, ns_df = 3,
                                  B = 200, n_pseudo = 10000) {

  n <- nrow(Data_cgp)
  if (n < 50) {
    return(list(
      AF = c(AF_X_L=NA, AF_X_U=NA, AF_Age_L=NA, AF_Age_U=NA, AF_Sex_L=NA, AF_Sex_U=NA,
             AF_READ_L=NA, AF_READ_U=NA, AF_COADREAD_L=NA, AF_COADREAD_U=NA),
      n_ok = 0
    ))
  }
  ...
  for (b in 1:B) {
    ...
    # Fit T1 (weibull) and T2 (gengamma)
    d_b$T1_event <- 1
    ...
    fit_t1_b <- tryCatch(flexsurvreg(form_t1, data = d_b, weights = d_b$iptw, dist = "weibull"),
                         error = function(e) NULL)
    fit_t2_b <- tryCatch(flexsurvreg(form_t2, data = d_b, weights = d_b$iptw, dist = "gengamma"),
                         error = function(e) NULL)
    if (is.null(fit_t1_b) || is.null(fit_t2_b)) next

    # G-computation (Total Effect): sample pseudo cohort from observed profiles with iptw
    ...
    # final marginal model (keep same as your pipeline: llogis)
    form_final <- as.formula(paste("Surv(sim_OS, sim_Event) ~", paste(valid_covs, collapse = " + ")))
    fit_final_b <- tryCatch(flexsurvreg(form_final, data = p_b, dist = "llogis"),
                            error = function(e) NULL)
    if (is.null(fit_final_b)) next

    af_b <- extract_af_full(fit_final_b)

    # store only non-NA
    if (!is.na(af_b["AF_X_est"])) store$AF_X <- c(store$AF_X, af_b["AF_X_est"])
    if (!is.na(af_b["AF_Age_est"])) store$AF_Age <- c(store$AF_Age, af_b["AF_Age_est"])
    if (!is.na(af_b["AF_Sex_est"])) store$AF_Sex <- c(store$AF_Sex, af_b["AF_Sex_est"])
    if (!is.na(af_b["AF_READ_est"])) store$AF_READ <- c(store$AF_READ, af_b["AF_READ_est"])
    if (!is.na(af_b["AF_COADREAD_est"])) store$AF_COADREAD <- c(store$AF_COADREAD, af_b["AF_COADREAD_est"])
  }
  ...
  ci_X <- q025_975(store$AF_X)
  ci_A <- q025_975(store$AF_Age)
  ci_S <- q025_975(store$AF_Sex)
  ci_R <- q025_975(store$AF_READ)
  ci_C <- q025_975(store$AF_COADREAD)

  list(
    AF = c(AF_X_L = ci_X[1], AF_X_U = ci_X[2],
           AF_Age_L = ci_A[1], AF_Age_U = ci_A[2],
           AF_Sex_L = ci_S[1], AF_Sex_U = ci_S[2],
           AF_READ_L = ci_R[1], AF_READ_U = ci_R[2],
           AF_COADREAD_L = ci_C[1], AF_COADREAD_U = ci_C[2]),
    n_ok = min(length(store$AF_X), length(store$AF_Age), length(store$AF_Sex), length(store$AF_READ), length(store$AF_COADREAD))
  )
}

# =========================================================
# Core Simulation Engine
#   - unchanged DGP / censoring / 1:5 calibration / gengamma / Gcomp
#   - only change: Proposed CI via bootstrap(B=200)
# =========================================================
run_sim_iteration <- function(N_target, True_AF_X, Mut_Freq, True_Med, True_Shape,
  ...
  # --- Baseline comparator models (unchanged) ---
  ...
  # --- Step 2: Decoupled AFT (unchanged except T2 dist already gengamma) ---
  # T1: Weibull
  Data_cgp$T1_event <- 1
  form_t1 <- as.formula(paste("Surv(T1, T1_event) ~", paste(valid_covs, collapse = " + ")))
  fit_t1 <- tryCatch(
  ...
  # --- Step 3: Calibrated G-computation (Total Effect; unchanged) ---
  ...
  # final marginal model for effect extraction (keep as llogis)
  form_prop_final <- as.formula(paste("Surv(sim_OS, sim_Event) ~", paste(valid_covs, collapse = " + ")))
  fit_prop_final <- tryCatch(flexsurvreg(form_prop_final, data = pseudo_cgp, dist = "llogis"),
                             error = function(e) NULL)

  # --- Marginal metrics and curves (unchanged style) ---
  ...
  calc_marginal_metrics <- function(sim_times) {
    ...
    c(Med = median(sim_times, na.rm = TRUE) / 365.25,
  ...
  # =========================================================
  # Proposed AF (point estimate) + Bootstrap CI (B=200)
  # =========================================================
  prop_point <- extract_af_full(fit_prop_final)

  # Bootstrap CI (percentile) for Proposed (Total Effect)
  # NOTE: this is the only intentional change for CI width / CR.
  boot <- bootstrap_proposed_ci(
    Data_cgp = Data_cgp,
  ...
  # overwrite only CI fields with bootstrap CI (keep point estimates)
  Prop_AF <- prop_point
  Prop_AF["AF_X_L"] <- boot$AF["AF_X_L"]; Prop_AF["AF_X_U"] <- boot$AF["AF_X_U"]
  Prop_AF["AF_Age_L"] <- boot$AF["AF_Age_L"]; Prop_AF["AF_Age_U"] <- boot$AF["AF_Age_U"]
  Prop_AF["AF_Sex_L"] <- boot$AF["AF_Sex_L"]; Prop_AF["AF_Sex_U"] <- boot$AF["AF_Sex_U"]
  Prop_AF["AF_READ_L"] <- boot$AF["AF_READ_L"]; Prop_AF["AF_READ_U"] <- boot$AF["AF_READ_U"]
  Prop_AF["AF_COADREAD_L"] <- boot$AF["AF_COADREAD_L"]; Prop_AF["AF_COADREAD_U"] <- boot$AF["AF_COADREAD_U"]
  ...
  out <- list(
    ...
    # Proposed (Total Effect) with bootstrap CI
    Prop_AF = Prop_AF,
...
#   (NOTE: This part is pasted as-is from your current script.
#          Proposed label can be changed to "Proposed..Total.Effect." if desired,
#          but I left the structure intact.)
# -------------------------------------------------------------------------
...
observeEvent(input$run_sim, {
  ...
  output$sim_result_table <- renderTable({
    data.frame(
      ...
      `Standard LT AFT` = c(format_af_single(res$LT_AF, "AF_X"),
                            ...
                            safe_fmt(input$sim_n, 0)),
      `Proposed (Total Effect, Bootstrap CI)` = c(paste0("<b>", format_af_single(res$Prop_AF, "AF_X"), "</b>"),
  ...
  output$sim_survival_plot <- renderPlot({
    t_seq_year <- seq(0, 5, length.out = 100)
    plot_df <- data.frame(
      Time = rep(t_seq_year, 4),
      Survival = c(res$curve_true, res$curve_naive, res$curve_lt, res$curve_prop),
      Model = factor(rep(c("1. True Marginal (Macro)", "2. Naive AFT (Immortal Bias)",
                           "3. Standard LT (Dep. Trunc Bias)", "4. Proposed (Total Effect)"),
                         each = 100))
    )
...
observeEvent(input$run_sim_multi, {
  ...
  output$sim_multi_result_table <- renderTable({
    data.frame(
      ...
      `Standard LT AFT` = c(calc_stats_af("LT_AF", "AF_X", True_AF),
                            ...
                            "N/A"),
      `Proposed (Total Effect, Bootstrap CI)` = c(paste0("<b>", calc_stats_af("Prop_AF", "AF_X", True_AF), "</b>"),
  ...
  # Figures (unchanged from your current script; omitted here for brevity in output)
  # If you want, paste your Fig1-3 blocks exactly as they were.
})
