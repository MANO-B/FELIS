...
#  - Nelder-Mead Optimization for IPTW to prevent gradient vanishing
#  - T1 & T2 models: gengamma for enhanced representation
#  - Estimate TOTAL effect on OS by Counterfactual Distributions
#  - Adjusted CI using ESS
#  - Strict Inf / NA Error prevention & unname() fix
# =========================================================================
...
# Fixed: Using Nelder-Mead to overcome flat gradient in KM steps
# -------------------------------------------------------------------------
calculate_calibrated_iptw <- function(data, ref_surv_list) {
  ...
  for (ag in unique(data$Age_class)) {
    ...
    obj_func <- function(theta) {
      ...
      S_est[is.na(S_est)] <- 0 # Handle early end of follow-up

      sum((S_est - S_macro_target)^2) + 1e-5 * sum(theta^2)
    }

    # Changed from BFGS to Nelder-Mead for stepwise functions
...
gen_sim_times_safe <- function(fit, newdata, dist_type = NULL) {
  ...
  for (r in rownames(res_m)) {
    if (grepl("^ns\\d+$", r) && r %in% names(newdata)) xb <- xb + res_m[r, "est"] * ind(newdata[[r]])
  }

  sim_t <- rep(NA_real_, n)

  # Strictly cap the location parameter to prevent Inf OS
  ...
  if (dist_type %in% c("weibull", "weibullPH", "weibullph")) {
    ...
    if (all(c("mu","sigma","Q") %in% rownames(res_m))) {
      mu0   <- as.numeric(res_m["mu", "est"])
      sigma <- as.numeric(res_m["sigma", "est"])
      Q     <- as.numeric(res_m["Q", "est"])
      if (is.finite(sigma) && sigma > 0) {
        mu <- safe_mu(mu0)
        sim_t <- flexsurv::rgengamma(n, mu = mu, sigma = sigma, Q = Q)
      }
    }
  }

  # Replace invalid outcomes generated by random engines
  if (all(is.na(sim_t))) {
    sim_t <- rep(365.25, n)
  } else {
  ...
  pmin(sim_t, 365.25 * 100) # Hard cap at 100 years
}

# -------------------------------------------------------------------------
# Counterfactual AF Calculator (Using Delta Method and ESS Scaling)
# -------------------------------------------------------------------------
ratio_ci_from_samples <- function(x0, x1, n_eff) {
  ...
  calc_se_log_med <- function(x, m) {
    ...
    return( 1.2533 * s / sqrt(n_eff) )
  }

  se_log_m0 <- calc_se_log_med(x0, m0)
  se_log_m1 <- calc_se_log_med(x1, m1)
...
compute_prop_afs_counterfactual <- function(pseudo_base, fit_t1, fit_t2, fit_t1res, ns_obj, ns_colnames, Data_cgp_logT1_resid, n_eff) {
  ...
  sim_os_do <- function(df) {
    df$sim_T1 <- gen_sim_times_safe(fit_t1, df)
    ...
    if (!is.null(fit_t1res)) {
      ...
      if (!is.null(pred) && length(pred) == nrow(df) && all(is.finite(pred))) df$logT1_sim_resid <- df$logT1_sim_scale - pred
    }

    # CLAMP: Prevent extrapolation explosion
    df$logT1_sim_resid <- pmax(min(Data_cgp_logT1_resid, na.rm = TRUE), pmin(max(Data_cgp_logT1_resid, na.rm = TRUE), df$logT1_sim_resid))

    ns_sim <- tryCatch(predict(ns_obj, df$logT1_sim_resid), error = function(e) NULL)
    if (is.null(ns_sim)) { ns_sim <- matrix(0, nrow(df), length(ns_colnames)) } else { ns_sim <- as.matrix(ns_sim) }
    colnames(ns_sim) <- ns_colnames
    for (j in seq_along(ns_colnames)) df[[ns_colnames[j]]] <- ns_sim[, j]

    df$sim_T2 <- gen_sim_times_safe(fit_t2, df)
  ...
  # UNNAME is crucial to prevent "AF_X_est.est" naming conflicts that lead to N/A
  Prop_AF <- c(
...
run_sim_iteration <- function(N_target, True_AF_X, Mut_Freq, True_Med, True_Shape, t1_pat, cens_pat, cens_rate, return_data = FALSE) {
  ...
  # Try Gengamma first for T1 model to improve representation flexibility
  fit_t1 <- tryCatch(flexsurvreg(form_t1, data = Data_cgp, weights = Data_cgp$iptw, dist = "gengamma"), error = function(e) NULL)
  if (is.null(fit_t1)) {
  ...
  cf <- tryCatch(
    compute_prop_afs_counterfactual(
      pseudo_base = pseudo, fit_t1 = fit_t1, fit_t2 = fit_t2, fit_t1res = fit_t1res,
      ns_obj = ns_obj, ns_colnames = colnames(ns_mat), Data_cgp_logT1_resid = Data_cgp$logT1_resid, n_eff = ESS
    ), error = function(e) NULL
  )
