...
output$figure_survival_CTx_interactive_1_control = renderPlot({
  ...
  # --- Proposed (Simulation-identical): T1/T2 + ns(logT1) + G-comp ---
  ...
  km <- survival::survfit(survival::Surv(time_all, censor) ~ Group, data = Data_survival_simulated)
  med_days <- summary(km)$table[, "median"]
  med_month <- med_days / 365.25 * 12
  message(sprintf("[RESULT] Median OS (months): %s",
                  paste(names(med_month), sprintf("%.1f", med_month), sep="=", collapse=" | ")))
  Data_survival_simulated$event <- 1  # 擬似OSは admin で0/1付けてるので不要なら省略可
  aft_fit <- flexsurv::flexsurvreg(survival::Surv(time_all, censor) ~ Group,
                                   data = Data_survival_simulated, dist = "llogis")
  print(aft_fit$res)
  # TR(=AF) は exp(beta)（Groupの係数）
  ...
  shiny::validate(shiny::need(nrow(Data_model %>% dplyr::filter(Group == "EP0")) >= 5,
                              "Not enough pts without recommended treatment to establish the reference baseline."))
  ...
  survival_compare_and_plot_CTx(
    ...
    status_var = "censor",
    group_var = "Group",
    plot_title = "Standardized OS (Simulation-identical: Calibrated G-computation)",
    adjustment = FALSE,
...
fit_and_simulate_proposed_real <- function(Data_model, covariates, ns_df = 3, M_pseudo = 5000, cap_years = 10) {
  ...
  # simulate EP0/1/2 all (keep same Group)
  for (g in levels(pseudo_base$Group)) {
    pseudo_g <- pseudo_base
    pseudo_g$Group <- factor(g, levels = levels(pseudo_base$Group))
  ...
  simdat <- dplyr::bind_rows(sim_list) %>%
    ...
    dplyr::mutate(Group = factor(Group, levels = levels(pseudo_base$Group)))
