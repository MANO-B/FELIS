...
```
install.packages(c('ggplot2', 'umap', 'tidyr', 'dbscan', 'shinyWidgets', 'readr', 'dplyr', 'stringr', 'RColorBrewer', 'gt', 'gtsummary', 'flextable', 'Rediscover', 'survival', 'gridExtra', 'survminer', 'tranSurv', 'DT', 'ggsci', 'scales', 'patchwork', 'sjPlot', 'sjlabelled', 'forcats', 'markdown','PropCIs','shinythemes', 'zoo', 'BiocManager'), dependencies = TRUE)
...
- 各組織型のCGP検査までの期間と治療についての情報の関係を**CGPまでの期間と治療到達率**タブに表示します。  
    CGP検査までの期間とEvidence level A, B以上, C以上の薬剤がある割合を散布図としました。  
...
- 組織型、PS、遺伝子変異の有無で群分けをした生存期間解析を**CGP後の生存期間とPS**タブに表示します。  
...
- CGP検査後の死亡に関するハザード比のforest plotを**CGP後の生存期間のHazard ratio, forest plot**タブに表示します。  
    ...
    死亡イベントがない因子がある場合は結果が表示されません。  
- **推奨治療がある患者が**推奨治療を受けられるかの要因についてのロジスティクス回帰分析の結果をforest plotとして**推奨治療を受ける要因, forest plot**タブに表示します。  
    全く推奨治療にたどり着いていない因子がある場合には数値が表示されません。  
- **推奨治療がある患者が**推奨治療を受けられるかの要因についてのサマリーの表を**推奨治療を受ける要因, 表**タブに表示します。  
